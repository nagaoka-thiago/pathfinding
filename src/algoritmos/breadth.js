...
const construct_path = (start_x, start_y, finish_x, finish_y, path) => {
    if(start_x === finish_x && start_y === finish_y) return []
    const [x, y] = path.get(`${start_x}, ${start_y}`)
    const newPath = [[x, y]]
    return newPath.concat(construct_path(x, y, finish_x, finish_y, path))
}
...
const breadth_step = (grid, setGrid, finish_x, finish_y) => {
    ...
    if(queue.length > 0) {
        ...
        if(current_x === finish_x && current_y === finish_y) return [true, [current_x, current_y], construct_path(cx, cy, finish_x, finish_y, path)]
